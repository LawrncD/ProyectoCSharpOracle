...
namespace MiProyectoCSharp.Repository
{
    public class JugadorDAO
    {
        public List<Jugador> ObtenerTodos()
        {
            ...
            string sql = "SELECT j.id_jugador, j.nombre, j.fecha_nacimiento, j.posicion, j.peso, j.estatura, j.valor_mercado, e.id_equipo, e.nombre, e.pais, e.valor FROM Jugador j LEFT JOIN Equipo e ON j.id_equipo = e.id_equipo ORDER BY j.id_jugador DESC";
            ...
            while (reader.Read())
            {
                lista.Add(new Jugador
                {
                    ...
                    Equipo = new Equipo
                    {
                        ...
                        ValorTotalEquipo = Convert.ToDecimal(reader["valor"])
                    }
                });
            }
            return lista;
        ...
        public bool Insertar(Jugador jugador)
        {
            ...
            cmd.Parameters.Add(new OracleParameter("equipo", jugador.Equipo.IdEquipo));

            return cmd.ExecuteNonQuery() > 0;
        }

        // 1. Determinar los datos del jugador m�s costoso por confederaci�n  
        ...
        // 2. Determinar la cantidad de jugadores por equipo que tienen menos de 21 a�os.
        ...
        // Reporte: Listar los jugadores cuyo peso, estatura y equipo est�n dentro de lo solicitado.
